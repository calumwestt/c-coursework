...
int col(char *text) {
    if (strchr(text, '1') != NULL) return 0;
    if (strchr(text, '2') != NULL) return 1;
    else return 2;
}
...
player win(game *g) {
    ...
    if (winner == N) winner = line(g -> grid[1][0], g -> grid[1][1], g -> grid[1][2]);
    if (winner == N) winner = line(g -> grid[2][0], g -> grid[2][1], g -> grid[2][2]);
    if (winner == N) winner = line(g -> grid[0][0], g -> grid[1][0], g -> grid[2][0]);
    if (winner == N) winner = line(g -> grid[0][1], g -> grid[1][1], g -> grid[2][1]);
    if (winner == N) winner = line(g -> grid[0][2], g -> grid[1][2], g -> grid[2][2]);
    if (winner == N) winner = line(g -> grid[0][0], g -> grid[1][1], g -> grid[2][2]);
    if (winner == N) winner = line(g -> grid[0][2], g -> grid[1][1], g -> grid[2][0]);
    return winner;
}
...
bool draw(game *g) {
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) {
        if (g -> grid[i][j] == X || g -> grid[i][j] == O) cells_occupied++;
      }
    }
    if (cells_occupied == 9) return true;
    else return false;
}
