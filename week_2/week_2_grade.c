...
int convert(int n, const char mark[]) {
    if (n == 1) {
      ...
      if (isdigit(mark[0]) && isdigit(mark[1])) {
        return (atoi(mark));
      }
    }
    else if (n == 3) {
      if (isdigit(mark[0]) && isdigit(mark[1]) && isdigit(mark[2])) {
...
int digit(const char mark[]) {
    return 0;
}

// Convert a mark string into a grade.
int grade(int m) {
    if (m >= 0 && m <= 39) return Fail;
    else if (m > 39 && m <= 49) return G3;
    else if (m > 49 && m <= 59) return G22;
    else if (m > 59 && m <= 69) return G21;
    else if (m > 69 && m <= 100) return G1;
    else return Invalid;
}
